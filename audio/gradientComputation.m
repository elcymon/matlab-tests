...
% lineRegEq is based on equation found in https://tinyurl.com/y7r6e4ej
...
lineRegEq = NaN(size(PulsFilterData,1)-1,Iters);
...
%                 bluelineRegEq = bluelineRegEq + sum(tvRegDiffgrads==-1);
%                 redlineRegEq = redlineRegEq + sum(tvRegDiffgrads==1);
            end
    ...
    %     pause
        ...
        lineRegEq(i1,ii) = bluelineRegEq/(redlineRegEq+bluelineRegEq) * 100;
...
LineFitFilterCells = arrayfun(@(MN,STD) sprintf('%.2f $%s$ %.2f',MN,'\pm',STD),mean(LineFitFilter,2),std(LineFitFilter,0,2),...
                                'UniformOutput',false);
TVRegDiffCells = arrayfun(@(MN,STD) sprintf('%.2f $%s$ %.2f',MN,'\pm',STD),mean(lineRegEq,2),std(lineRegEq,0,2),...
                                'UniformOutput',false);
